...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void MovePlayer()
    {
        if (_vertInput > 0) // Pressing ↓ made the player literally jump backwards (so it's needed the _screenUp variable)
        {
            ...
            if (!isDummy)
                transform.LookAt((_screenRight * _horizInput + _screenForward * _vertInput).normalized, transform.up); // Rotates the player in the input direction
        }
        else
        {
            ...
            if (!isDummy)
                transform.LookAt((_screenRight * _horizInput + _screenUp * _vertInput).normalized, transform.up); // Same, but using _screenUp to rotate correctly backwards
        }
