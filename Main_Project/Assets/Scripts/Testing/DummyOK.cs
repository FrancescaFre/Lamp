...
public class DummyOK : MonoBehaviour
{
    ...
    */
    ...
    private void MovePlayer()
    {
        if (_vertInput > 0) // Pressing ↓ made the player literally jump backwards (so it's needed the _screenUp variable)
            _movement = (_screenRight * _horizInput + _screenForward * _vertInput).normalized * _stepSpeed * Time.deltaTime; // Takes camera axes to correct the direction
        else
            _movement = (_screenRight * _horizInput + _screenUp * _vertInput).normalized * _stepSpeed * Time.deltaTime; // Same, but using _screenUp to go backwards correctly
    ...
    */
    ...
    private void PleaseMoveIBegYou()
    {
        ...
        if (_vertInput > 0)
            _movement = (_screenRight * _horizInput + _screenForward * _vertInput).normalized * _walkSpeed * Time.deltaTime;
        else
            _movement = (_screenRight * _horizInput + _screenUp * _vertInput).normalized * _walkSpeed * Time.deltaTime;
